...
7. **score.js** — `scoreAnalysis(analysis)` turns findings into 0–10 scores per dimension + the overkill index, then `chooseVerdict` maps scores→verdict.
8. **report.js** — `renderMarkdownReport(report)` for human output; `--json` bypasses it and dumps the report object.
