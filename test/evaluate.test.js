...
test('per-component claudeMdHazards survive into each component\'s own analysis in a multi-component rollup', async () => {
  const fetchImpl = async (url) => {
    ...
    if (url.endsWith('git/trees/main?recursive=1')) {
      return jsonResponse({ tree: [
        { path: '.claude-plugin/marketplace.json', type: 'blob', size: 40 },
        { path: 'plugins/a/.claude-plugin/plugin.json', type: 'blob', size: 20 },
        { path: 'plugins/a/fixtures/malicious_claude_md/CLAUDE.md', type: 'blob', size: 20 },
        { path: 'plugins/b/.claude-plugin/plugin.json', type: 'blob', size: 20 },
      ] });
    }
    if (url.endsWith('/contents/.claude-plugin/marketplace.json')) {
    ...
    if (url.endsWith('/contents/plugins/a/.claude-plugin/plugin.json')) return contentsResponse({ name: 'a' });
    if (url.endsWith('/contents/plugins/a/fixtures/malicious_claude_md/CLAUDE.md')) {
  ...
  assert.ok(pluginA.findings.some((f) => f.id === 'candidate-instruction-bomb'), 'plugin a should have the instruction-bomb finding');
  assert.ok(!pluginB.findings.some((f) => f.id === 'candidate-instruction-bomb'), 'plugin b must not inherit plugin a\'s claudeMdHazards');
});
